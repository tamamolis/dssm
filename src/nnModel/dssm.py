...
class Model(object):
    ...
    def __init__(self, sample=11, sample_nums=200, lr=0.1, lrdf=0.95, l1=1000, l2=240, l3=400, cks=[2, 3, 4],
                 ckn=[128, 64, 64]):
        self.SAMPLE = sample
        ...
        with tf.device(device_name):
            ...
            with tf.name_scope('convolution_layer'):
                ...
                for i, size in enumerate(self.conv_kernel_size):
                    ...
                    self.query_conv[size] = self.__conv2d('conv' + str(size), self.query_words_out, self.wc[size],
                                                          self.bc[size])
                    self.query_pool[size] = self.__full_max_pool('pool' + str(size), self.query_conv[size],
                                                                 [0, 3, 2, 1])
                    self.query_pool_list.append(self.query_pool[size])

                    self.doc_conv[size] = self.__conv2d('conv' + str(size), self.doc_words_out, self.wc[size],
                                                        self.bc[size])
                ...
                self.query_conv_out = tf.nn.l2_normalize(
                    tf.nn.relu(tf.reshape(self.query_pool_merge, [-1, sum(self.conv_kernel_number)])), 1)

                self.doc_pool_merge = tf.concat(self.doc_pool_list, 3)
                self.doc_conv_out = tf.nn.l2_normalize(
                    tf.nn.relu(tf.reshape(self.doc_pool_merge, [-1, sum(self.conv_kernel_number)])),
                    1)

            with tf.name_scope('dense_layer_1'):
                self.l1_par_range = np.sqrt(6.0 / (sum(self.conv_kernel_number) + self.L1_N))
                self.wd1 = tf.Variable(
                    tf.random_uniform([sum(self.conv_kernel_number), self.L1_N], -self.l1_par_range, self.l1_par_range))
            ...
            with tf.name_scope('loss'):
                ...
                self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.out, labels=self.label))
