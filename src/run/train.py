        ...
        for epoch in range(FLAGS.max_epoch):
            for step in range(epoch_steps):
                ...
                mgd, ls, acc, pred_, label_, l_rate = sess.run(
                    [dssmModel.merged, dssmModel.loss, dssmModel.accuracy, dssmModel.pred_y, dssmModel.label_y,
                     dssmModel.learning_rate],
                    feed_dict={dssmModel.query: query_batch, dssmModel.doc: doc_batch, dssmModel.label: label_batch})
                p_p, p_r, n_p, n_r = biclass_rate(label_, pred_)
                train_writer.add_summary(mgd, epoch * epoch_steps + step)
                print(
                    'Train Epoch %d, Step %d, l_rate: %f, loss: %f, accuracy: %f, p_precision: %f, p_recall: %f, n_precision: %f, n_recall: %f' % (
                    epoch + 1, step + 1, l_rate, ls, acc, p_p, p_r, n_p, n_r))
                sys.stdout.flush()

                if step % 5 == 0:
                    ls, acc, pred_, label_ = sess.run(
                        [dssmModel.loss, dssmModel.accuracy, dssmModel.pred_y, dssmModel.label_y],
                        feed_dict={dssmModel.query: valid_query, dssmModel.doc: valid_doc,
                                   dssmModel.label: valid_label})
                    p_p, p_r, n_p, n_r = biclass_rate(label_, pred_)
                    if len(previous_losses) >= 5 and ls > max(previous_losses[-5:]):
                        ...
                        epoch + 1, step + 1, dssmModel.learning_rate.eval()))
                    previous_losses.append(ls)
                    print(
                        'Valid Epoch %d, loss: %f, accuracy: %f, p_precision: %f, p_recall: %f, n_precision: %f, n_recall: %f' % (
                        epoch + 1, ls, acc, p_p, p_r, n_p, n_r))
                    sys.stdout.flush()
                    if ls < max_loss:
                        dssmModel.save_model(saver, sess, FLAGS.model_path)

            ls, acc, pred_, label_ = sess.run([dssmModel.loss, dssmModel.accuracy, dssmModel.pred_y, dssmModel.label_y],
                                              feed_dict={dssmModel.query: test_query, dssmModel.doc: test_doc,
                                                         dssmModel.label: test_label})
            p_p, p_r, n_p, n_r = biclass_rate(label_, pred_)
            print("--------------------------------------------")
            print(
                'Test Epoch %d, loss: %f, accuracy: %f, p_precision: %f, p_recall: %f, n_precision: %f, n_recall: %f' % (
                epoch + 1, ls, acc, p_p, p_r, n_p, n_r))
            print("--------------------------------------------")
            sys.stdout.flush()
